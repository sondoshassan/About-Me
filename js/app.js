...
var qSix=function(){
  var age = prompt ('how much am I old?','using number [15,30]');
  age = Number(age);
  var i = 0;
  if (age === 23){
    alert ('yes I\'m in 23');
  ...
  while (age !== 23 && i<3){
    ...
    var age = prompt ('how much am I old?','using number');
    age = Number(age);
    if (age === 23)
    {
      alert ('yes I\'m in 23');
...
var qSeven =function(){
  ...
  var userGuess = prompt ('what are my languages programming?','c++, javascript, java, html, css');
  ...
  for (var k=0; k < 5;k++){
    ...
    else if (userGuess === langPrograme[l]) {
      break;
    }
  }
  //   console.log ('k out loop = '+ k);
  if (userGuess !== langPrograme[l] && k === 5){
    alert ('thats wrong');
  }
